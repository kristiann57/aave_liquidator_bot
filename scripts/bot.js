...
if (mode === 'testnet') {
    ...
    DB_BORRWERS = process.env.DB_BORROWERS_OPTIMISM_MAINNET; // Select DB 6
    UniswapV3SwapRouter = "0xE592427A0AEce92De3Edee1F18E0157C05861564"; // Mainnet OP address
    ...
    BrazoEjecutorAddress = process.env.BRAZO_EJECUTOR_OP_MAINNET;
}
...
async function estimateGasCost(collateralAsset, debtAsset,liquidatableUser, debtToCover, gasPrice) {
  ...
  let gasEstimate;
  try {
    gasEstimate = await BRAZO_EJECUTOR_CONTRACT_WITH_SIGNER.estimateGas.liquidate(
      collateralAsset, 
      debtAsset, 
      liquidatableUser, 
      debtToCover
    ...
    console.log('ESTIMATEGASCOST -- ERROR -- : Error:', error) 
    console.log('ESTIMATEGASCOST -- ERROR -- : CollateralASset:', collateralAsset)
    console.log('ESTIMATEGASCOST -- ERROR -- : DebtAsset:', debtAsset)
    console.log('ESTIMATEGASCOST -- ERROR -- : LiquidatableUser:', liquidatableUser)
    console.log('ESTIMATEGASCOST -- ERROR -- : DebtToCover:', debtToCover)
    return 0;
  }
...
async function scanAndProcessAddresses() {
    do {
        ...
        if (liquidatableUsers.length ==  1) {
          ...
          if (estimatedGasCostEth == 0) continue ; // if the estimate fails, continue to the next iteration of the loop

          console.log('SCAN: Estimated gas cost(single user):', estimatedGasCostEth) /// **** REMOVE THIS LINE LATER ****
          ...
          for (let i = 0; i < liquidatableUsersAddresses.length; i++) { 
            ...
            console.log('SCAN: Estimated gas cost(multiple users found):', estimatedGasCostEth) /// **** REMOVE THIS LINE LATER ****
